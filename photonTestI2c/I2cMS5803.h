...
public:
	...
	uint32_t  convertTemperature(uint32_t rawValue) {
		...
		if (temp_calc < 2000) {    // If temp_calc is below 20.0C
			...
			_OFF2 = 3 * ((temp_calc - 2000) * (temp_calc - 2000)) / 2;
			_SENS2 = 5 * ((temp_calc - 2000) * (temp_calc - 2000)) / 8;

			if(temp_calc < -1500) {       // If temp_calc is below -15.0C
				_OFF2 = _OFF2 + 7 * ((temp_calc + 1500) * (temp_calc + 1500));
				_SENS2 = _SENS2 + 4 * ((temp_calc + 1500) * (temp_calc + 1500));
			}
		}
		else {      // If temp_calc is above 20.0C
			//T2 = 7 * ((uint64_t)dT * dT)/pow(2,37);
			T2 = 7 * ((uint64_t)dT * dT) >> 37;
			_OFF2 = ((temp_calc - 2000) * (temp_calc - 2000)) / 16;
			_SENS2 = 0;
		}
